...
begin
  ...
  if p.is_active is not true or p.profile_completed is not true then
    raise exception 'Perfil incompleto o inactivo';
  end if;
  if m.home_team_id is null or m.away_team_id is null then
    raise exception 'No se puede pronosticar equipos Por definir';
  end if;
  if m.lock_at is null or now() >= m.lock_at or m.manually_locked or m.status in ('LOCKED','LIVE','FINISHED','CANCELLED') then
    raise exception 'Pronóstico cerrado';
  end if;
  if new.predicted_outcome = 'DRAW' and m.round <> 'THIRD_PLACE' and new.predicted_qualified_team_id is null then
    raise exception 'Seleccione quién clasifica';
  end if;
  if new.predicted_qualified_team_id is not null
     and new.predicted_qualified_team_id <> m.home_team_id
     and new.predicted_qualified_team_id <> m.away_team_id then
    raise exception 'El clasificado debe ser uno de los dos equipos del partido';
  end if;

  if TG_OP = 'INSERT' then
    new.result_points := 0;
    new.qualified_points := 0;
    new.total_points := 0;
    new.calculation_status := 'PENDING';
  elsif TG_OP = 'UPDATE' then
    new.result_points := old.result_points;
    new.qualified_points := old.qualified_points;
    new.total_points := old.total_points;
    new.calculation_status := old.calculation_status;
  end if;
...
begin
  ...
  v_winner := m.qualified_team_id;
  v_loser := case
    when m.home_team_id = m.qualified_team_id then m.away_team_id
    when m.away_team_id = m.qualified_team_id then m.home_team_id
    else null
  end;
  ...
  set result_points = case
        when p.predicted_home_score = m.home_score_90 and p.predicted_away_score = m.away_score_90 then 3
        when (p.predicted_home_score > p.predicted_away_score and m.home_score_90 > m.away_score_90)
          or (p.predicted_home_score < p.predicted_away_score and m.home_score_90 < m.away_score_90)
          or (p.predicted_home_score = p.predicted_away_score and m.home_score_90 = m.away_score_90) then 1
        else 0
      end,
      qualified_points = case when m.round <> 'THIRD_PLACE' and p.predicted_qualified_team_id = m.qualified_team_id then 1 else 0 end,
      total_points = case
        when p.predicted_home_score = m.home_score_90 and p.predicted_away_score = m.away_score_90 then 3
        when (p.predicted_home_score > p.predicted_away_score and m.home_score_90 > m.away_score_90)
          or (p.predicted_home_score < p.predicted_away_score and m.home_score_90 < m.away_score_90)
      ...
      end + case when m.round <> 'THIRD_PLACE' and p.predicted_qualified_team_id = m.qualified_team_id then 1 else 0 end,
      calculation_status = 'CALCULATED',
      updated_at = now()
  where p.match_id = p_match_id;

  update matches dependent
  set home_team_id = case
        when dependent.home_source_match_id = p_match_id and dependent.home_source_type = 'WINNER' then v_winner
        when dependent.home_source_match_id = p_match_id and dependent.home_source_type = 'LOSER' then v_loser
        else dependent.home_team_id
      end,
      away_team_id = case
        when dependent.away_source_match_id = p_match_id and dependent.away_source_type = 'WINNER' then v_winner
        when dependent.away_source_match_id = p_match_id and dependent.away_source_type = 'LOSER' then v_loser
        else dependent.away_team_id
      end,
      updated_at = now()
  where (dependent.home_source_match_id = p_match_id and dependent.home_source_type in ('WINNER','LOSER'))
     or (dependent.away_source_match_id = p_match_id and dependent.away_source_type in ('WINNER','LOSER'));

  insert into audit_logs(action, entity, entity_id, new_value)
  values('RECALCULATE', 'matches', p_match_id::text, jsonb_build_object('winner_team_id', v_winner, 'loser_team_id', v_loser));
end $$;
