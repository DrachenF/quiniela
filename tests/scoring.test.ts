...
it('propagación de ganador',()=>{const br={id:'br',name:'Brasil',shortName:'BRA',fifaCode:'BRA',isoCode:'BR',isActive:true};const fr={...br,id:'fr',name:'Francia'};const out=propagateResult([{id:'n',round:'FINAL',roundOrder:1,kickoffAt:'',lockAt:'',status:'SCHEDULED',homeSourceMatchId:'s',homeSourceType:'WINNER'} as any],{id:'s',round:'SEMI_FINAL',roundOrder:1,kickoffAt:'',lockAt:'',status:'FINISHED',homeTeam:br,awayTeam:fr,qualifiedTeamId:'br'} as any);expect(out[0].homeTeam?.id).toBe('br')});
it('propagación perdedor tercer puesto',()=>{const br={id:'br',name:'Brasil',shortName:'BRA',fifaCode:'BRA',isoCode:'BR',isActive:true};const fr={...br,id:'fr',name:'Francia'};const out=propagateResult([{id:'t',round:'THIRD_PLACE',roundOrder:1,kickoffAt:'',lockAt:'',status:'SCHEDULED',homeSourceMatchId:'s'} as any],{id:'s',round:'SEMI_FINAL',roundOrder:1,kickoffAt:'',lockAt:'',status:'FINISHED',homeTeam:br,awayTeam:fr,qualifiedTeamId:'br'} as any);expect(out[0].homeTeam?.id).toBe('fr')});
