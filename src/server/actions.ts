...
export async function saveMatchAction(_: unknown, formData: FormData) { const { sb, user } = await requireAdmin(); 

const raw: Record<string, FormDataEntryValue | null> =
  Object.fromEntries(formData); for (const k of ['home_team_id','away_team_id','qualified_team_id','kickoff_at','stadium','city']) if (raw[k] === '') raw[k] = null; const data = matchSchema.parse(raw); const { error, data: saved } = await sb.from('matches').upsert(data).select('id').single(); if (error) return { ok:false, message:error.message }; await sb.from('audit_logs').insert({ admin_user_id:user.id, action:'UPSERT_MATCH', entity:'matches', entity_id:saved.id, new_value:data }); revalidatePath('/admin'); return { ok:true, message:'Partido guardado.'}; }
export async function saveResultAction(_: unknown, formData: FormData) {
  ...
  try {
    ...
    const { data: match, error: matchError } = await sb
      .from('matches')
      .select('id,round,home_team_id,away_team_id,home_score_90,away_score_90,qualified_team_id,winner_team_id,loser_team_id,status')
      .eq('id', parsed.id)
      .single();
    ...
    return { ok:false, message:e instanceof Error ? e.message : 'Datos inválidos.' };
  }
}
